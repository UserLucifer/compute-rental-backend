...
public class ProductionConfigValidator implements SmartInitializingSingleton {
    ...
    private static final Duration MIN_PRODUCTION_AUTO_PAUSE_DELAY = Duration.ofHours(24);
    ...
    private void validateAutoPauseDelay(ArrayList<String> errors) {
        var value = property("app.order.auto-pause-delay");
        var delay = MIN_PRODUCTION_AUTO_PAUSE_DELAY;
        if (StringUtils.hasText(value)) {
            try {
                delay = DurationStyle.detectAndParse(value);
            } catch (IllegalArgumentException ex) {
                errors.add("Order auto pause delay must be a valid duration");
                return;
            }
        }
        if (delay.compareTo(MIN_PRODUCTION_AUTO_PAUSE_DELAY) < 0) {
            errors.add("Order auto pause delay must be at least 24h in production");
        }
    }
