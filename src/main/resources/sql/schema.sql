...
CREATE TABLE `rental_order` (
  ...
  KEY `idx_order_status_api_generated_at` (`order_status`, `api_generated_at`, `id`) COMMENT '用于部署费超时取消秒级扫描',
  KEY `idx_auto_pause_at` (`auto_pause_at`) COMMENT '用于自动暂停定时任务扫描',
  KEY `idx_order_status_auto_pause_at` (`order_status`, `auto_pause_at`, `id`) COMMENT '用于自动暂停秒级扫描',
  KEY `idx_created_at` (`created_at`),
